...
use crate::{Settings, Status, TimeStatus, TimeDataDaily, TrackerError};

#[derive(Default, Clone)]
pub struct TimeStatusDaily {
    data: Option<TimeDataDaily>,
    settings: Option<Settings>,
    start: Option<TimeStatus>,
    end: Option<TimeStatus>,
    temp_end: Option<TimeStatus>,
    /// first break of day
    f_break: Option<DateTime<Local>>,
    /// calculated break. whichever is higher break or exp_break.
    calc_break: Option<TimeStatus>,
    pub r#break: Option<TimeStatus>,
    pub exp_break: Option<TimeStatus>,

    online: Option<TimeStatus>,
    est_end: TimeStatus,

    pub worktime: TimeStatus,
    /// expected worktime for that day.
    exp_worktime: Option<TimeStatus>,
    ...
    fn set_end(&mut self) -> &mut Self {
            ...
            .find(|x| x.status == Status::End)
        {
            ...
            None => {
                ...
                self.temp_end = Some(TimeStatus::now())
            }
        };
        self
    }
...
pub struct StatusDailyBuilder {
    inner: TimeStatusDaily,
}

impl StatusDailyBuilder {
    pub fn data(&mut self, data: TimeDataDaily) -> &mut Self {
    ...
    pub fn build(&self) -> Result<TimeStatusDaily, TrackerError> {
...
impl std::fmt::Display for TimeStatusDaily {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let end_fmt = if end.is_some() {
            let f_break = self.f_break.map(TimeStatus::from);

            if f_break.is_some() {
                let e_break = TimeStatus::from(
                    self.f_break
                        .unwrap()
...
mod tests {
    ...
    use crate::Entry;
    use crate::{Settings, Status, TimeStatusDaily, TimeStatus, TimeData};
    ...
    mod display {
        ...
        use crate::{settings::BreakLimit, settings::WorkPerDayInMinutes};
